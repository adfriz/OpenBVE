...
namespace OpenBve.Graphics
{
	internal class NewRenderer : BaseRenderer
	{
		...
		public override void Initialize()
		{
			...
			Pipeline.AddPass(new LibRender2.Passes.ShadowDepthPass());
			Pipeline.AddPass(new LibRender2.Passes.SceneOpaquePass());
			Pipeline.AddPass(new LibRender2.Passes.AlphaPass());
			Pipeline.AddPass(new LibRender2.Passes.OverlayPass());
		}
		...
		internal void RenderScene(double TimeElapsed, double RealTimeElapsed)
		{
			...
			// Pipeline now handles everything
			Pipeline.Execute(TimeElapsed, RealTimeElapsed);
			// render touch
